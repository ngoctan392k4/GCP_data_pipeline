...
from schema_dict import create_schema
from column_analyzer import find_fields
from gcs_to_bigquery import normalize, upload_gcs, build_schema, create_dataset_table, load_bigquery
...
if __name__ == "__main__":
    ...
    find_fields(input_file="data/raw_data.jsonl", output_dir="bigquery/raw_layer_uploader/fields", output_file="field_types_raw.csv")

    logging.info(f"Analyzing column for product data")
    find_fields(input_file="data/product_info.jsonl", output_dir="bigquery/raw_layer_uploader/fields", output_file="field_types_product.csv")

    logging.info(f"Analyzing column for ip2location data")
    find_fields(input_file="data/ip_location_data.jsonl", output_dir="bigquery/raw_layer_uploader/fields", output_file="field_types_location.csv")

    #---------------------
    # Upload from GCS to Bigquery
    #---------------------
    #  Create schema file
    create_schema("bigquery/raw_layer_uploader/fields/field_types_raw.csv", "schema/bigquery_schema_raw.json")
    create_schema("bigquery/raw_layer_uploader/fields/field_types_product.csv", "schema/bigquery_schema_product.json")
    create_schema("bigquery/raw_layer_uploader/fields/field_types_location.csv", "schema/bigquery_schema_location.json")
    ...
    schema = build_schema(schema_file="schema/bigquery_schema_raw.json")
    ...
    logging.info(f"Building schema")
    schema = build_schema(schema_file="schema/bigquery_schema_product.json")
    logging.info(f"Creating dataset and table")
    ...
    logging.info(f"Building schema")
    schema = build_schema(schema_file="schema/bigquery_schema_location.json")
    logging.info(f"Creating dataset and table")
