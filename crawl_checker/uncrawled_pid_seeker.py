...
def invalid_product_id():
    files = os.listdir("data_enrichment/data/product_detail")
    crawl_pid = set()
    origin_pid = set()

    for file in tqdm(files, total=len(files)):
        with open(os.path.join("data_enrichment/data/product_detail", file), "r", encoding='utf-8') as rf:
            data = json.load(rf)
            for item in data:
    ...
    with open("data_enrichment/data/pid_url_in_total.json", "r", encoding='utf-8') as rf:
            data = json.load(rf)
